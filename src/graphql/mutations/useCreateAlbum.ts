import { generateClient, type GraphQLResult } from "aws-amplify/api";
import { gql } from "graphql-tag";
import { useState } from "react";
import type { Album } from "../types";

const CREATE_ALBUM = gql`
  mutation CreateAlbum($name: String!, $artistId: ID!) {
    albumCreate(name: $name, artistId: $artistId) {
      album {
        id
        artist {
          id
        }
      }
      imageUrl
      fields
    }
  }
`;

const client = generateClient();

interface CreateAlbumParams {
  name: string;
  artistId: string;
  file?: File;
}

type CreateAlbumResponse = {
  albumCreate: {
    album: Album;
    imageUrl: string;
    fields: string;
  };
};

export const useCreateAlbum = () => {
  const [isLoading, setIsLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);

  const createAlbum = async ({ name, artistId, file }: CreateAlbumParams) => {
    setIsLoading(true);
    setError(null);

    try {
      const response = await client.graphql({
        query: CREATE_ALBUM,
        variables: { name, artistId },
      }) as GraphQLResult<CreateAlbumResponse>;

      const { album, imageUrl, fields } = response.data.albumCreate;

      if (file && imageUrl && fields) {
        const parsedFields = typeof fields === 'string' ? JSON.parse(fields) : fields;
        await uploadToS3(imageUrl, parsedFields, file);
      }

      return album;
    } catch (err: unknown) {
      console.error("Create Album Error:", err);
      setError((err as Error).message || "Failed to create album");
      throw err;
    } finally {
      setIsLoading(false);
    }
  };

  return { createAlbum, isLoading, error };
};

async function uploadToS3(url: string, fields: Record<string, string>, file: File) {
  const formData = new FormData();

  Object.entries(fields).forEach(([key, value]) => {
    const s3Key = key.replace(/_/g, "-");
    formData.append(s3Key, value);
  });
  formData.append("Content-Type", file.type);
  formData.append("file", file);

  const response = await fetch(url, {
    method: "POST",
    body: formData,
  });

  if (!response.ok) {
    const errorXml = await response.text();
    console.error("S3 Upload Error XML:", errorXml);
    throw new Error("Failed to upload image to S3");
  }
}
